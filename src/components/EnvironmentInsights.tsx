...
export default function EnvironmentInsights() {
  ...
  const indicators: Indicator[] = [
    ...
    },
    {
      ...
      value: data.ghgEmissionsStats.reductionPercent,
      unit: '%',
      target: 20,
      targetCondition: '>=',
      lastUpdate: new Date(data.lastUpdated).toISOString().split('T')[0],
      source: 'NYC DEP',
      trend: data.ghgEmissionsStats.reductionPercent >= 20 ? 'down' : 'stable',
      color: '#06b6d4',
      higherIsBetter: false,
    ...
    },
    {
      ...
      value: data.recyclingDiversionStats.currentDiversionRate,
      unit: '%',
      target: 25,
      targetCondition: '>=',
      lastUpdate: new Date(data.lastUpdated).toISOString().split('T')[0],
      source: 'NYC DSNY',
      trend: data.recyclingDiversionStats.improvementPercent > 10 ? 'up' : 'stable',
      color: '#10b981',
      higherIsBetter: true,
  ...
  return (
    <div className="space-y-6">
      {/* Category Header */}
      <div className="mb-6">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2">Environment Sector</h2>
        <p className="text-gray-400 text-sm sm:text-base md:text-lg">Air quality, tree canopy, and environmental health metrics</p>
      </div>
      ...
      {/* Main Stats */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 items-start mb-6">
